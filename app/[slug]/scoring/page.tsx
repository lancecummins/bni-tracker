                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map(val => (
                          <button
                            key={val}
                            onClick={() => { const newVal = metrics.one21s === val ? '0' : val.toString(); handleMetricChange(member.id!, 'one21s', newVal); }}
                        ...
                        <input
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          value={tempInputs[member.id!]?.one21s ?? (metrics.one21s > 5 ? metrics.one21s : '')}
                          onChange={(e) => {
                            const val = e.target.value.replace(/\D/g, '');
                            setTempInputs(prev => ({
                              ...prev,
                              [member.id!]: { ...prev[member.id!], one21s: val }
                            }));
                          ...
                          onBlur={(e) => {
                            ...
                            setTempInputs(prev => {
                              const copy = { ...prev };
                              if (copy[member.id!]) delete copy[member.id!].one21s;
                              return copy;
                            });
                          }}
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((val) => (
                          <button
                            key={val}
                            onClick={() => {
                              const newVal = metrics.referrals === val ? '0' : val.toString();
                              handleMetricChange(member.id!, 'referrals', newVal);
                            }}
                            className={`flex-1 py-2 rounded text-sm font-medium transition-colors ${
                        ...
                        <input
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          value={tempInputs[member.id!]?.referrals ?? (metrics.referrals > 5 ? metrics.referrals : '')}
                          onChange={(e) => {
                            const val = e.target.value.replace(/\D/g, '');
                            setTempInputs(prev => ({
                              ...prev,
                              [member.id!]: { ...prev[member.id!], referrals: val }
                            }));
                            const num = parseInt(val) || 0;
                            if (num === 0 || num >= 6) {
                          ...
                          onBlur={(e) => {
                            ...
                            setTempInputs(prev => {
                              const copy = { ...prev };
                              if (copy[member.id!]) delete copy[member.id!].referrals;
                              return copy;
                            });
                          }}
                          onFocus={(e) => e.target.select()}
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((val) => (
                          <button
                            key={val}
                            onClick={() => {
                              const newVal = metrics.tyfcb === val ? '0' : val.toString();
                              handleMetricChange(member.id!, 'tyfcb', newVal);
                            }}
                            className={`flex-1 py-2 rounded text-sm font-medium transition-colors ${
                              metrics.tyfcb === val
                                ? 'bg-blue-600 text-white'
                        ...
                        <input
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          value={tempInputs[member.id!]?.tyfcb ?? (metrics.tyfcb > 5 ? metrics.tyfcb : '')}
                          onChange={(e) => {
                            const val = e.target.value.replace(/\D/g, '');
                            setTempInputs(prev => ({
                              ...prev,
                              [member.id!]: { ...prev[member.id!], tyfcb: val }
                            }));
                            const num = parseInt(val) || 0;
                            if (num === 0 || num >= 6) {
                          ...
                          onBlur={(e) => {
                            ...
                            setTempInputs(prev => {
                              const copy = { ...prev };
                              if (copy[member.id!]) delete copy[member.id!].tyfcb;
                              return copy;
                            });
                          }}
                          onFocus={(e) => e.target.select()}
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((val) => (
                          <button
                            key={val}
                            onClick={() => {
                              const newVal = metrics.visitors === val ? '0' : val.toString();
                              handleMetricChange(member.id!, 'visitors', newVal);
                            }}
                            className={`flex-1 py-2 rounded text-sm font-medium transition-colors ${
                        ...
                        <input
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          value={tempInputs[member.id!]?.visitors ?? (metrics.visitors > 5 ? metrics.visitors : '')}
                          onChange={(e) => {
                            const val = e.target.value.replace(/\D/g, '');
                            setTempInputs(prev => ({
                              ...prev,
                              [member.id!]: { ...prev[member.id!], visitors: val }
                            }));
                            const num = parseInt(val) || 0;
                            if (num === 0 || num >= 6) {
                          ...
                          onBlur={(e) => {
                            ...
                            setTempInputs(prev => {
                              const copy = { ...prev };
                              if (copy[member.id!]) delete copy[member.id!].visitors;
                              return copy;
                            });
                          }}
                          onFocus={(e) => e.target.select()}
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3, 4, 5].map((val) => (
                          <button
                            key={val}
                            onClick={() => {
                              const newVal = metrics.ceu === val ? '0' : val.toString();
                              handleMetricChange(member.id!, 'ceu', newVal);
                            }}
                            className={`flex-1 py-2 rounded text-sm font-medium transition-colors ${
                              metrics.ceu === val
                                ? 'bg-blue-600 text-white'
                        ...
                        <input
                          ...
                          inputMode="numeric"
                          pattern="[0-9]*"
                          value={tempInputs[member.id!]?.ceu ?? (metrics.ceu > 5 ? metrics.ceu : '')}
                          onChange={(e) => {
                            const val = e.target.value.replace(/\D/g, '');
                            setTempInputs(prev => ({
                              ...prev,
                              [member.id!]: { ...prev[member.id!], ceu: val }
                            }));
                            const num = parseInt(val) || 0;
                            if (num === 0 || num >= 6) {
                          ...
                          onBlur={(e) => {
                            ...
                            setTempInputs(prev => {
                              const copy = { ...prev };
                              if (copy[member.id!]) delete copy[member.id!].ceu;
                              return copy;
                            });
                          }}
                          onFocus={(e) => e.target.select()}
